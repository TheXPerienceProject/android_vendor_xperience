CLANG_VERSION=$(${ANDROID_BUILD_TOP}/vendor/xperience/tools/get_clang_version.py)
...
function eat() {
  if [ "$OUT" ]; then
    ZIPPATH=$(ls -tr "$OUT"/xperience-*.zip | tail -1)
    if [ ! -f $ZIPPATH ]; then
      echo "Nothing to eat"
      return 1
    fi
...
function cdevice() {
  T=$(ls -d $(gettop)/device/*/$( _get_build_var_cached TARGET_DEVICE))
...
function cloremote() {
  ...
  if [ $PROJECT = "vendor/qcom/opensource/commonsys/display" ]; then
    PROJECT="vendor/qcom-opensource/display-commonsys"
  fi
...
function clomerge() {
  ...
  if (echo $PROJECT | grep -q "vendor/qcom/opensource/commonsys/display"); then
    local BRANCH=${QSSIBRANCH}
    echo "Setting QSSI branch"
  fi
...
function installboot() {
  if [ ! -e "$OUT/recovery/root/system/etc/recovery.fstab" ]; then
  ...
  adb root
  adb wait-for-device-recovery
  if (adb shell getprop ro.xperience.device | grep -q "$XPERIENCE_BUILD"); then
...
function dopush() {
  ...
  if (adb shell getprop ro.xperience.device | grep -q "$XPERIENCE_BUILD") || [ "$FORCE_PUSH" = "true" ]; then
...
function generate_keys() {
    local subject="/C=MX/ST=Michoacan/L=LZC/O=TheXPerienceProject/OU=TheXPerienceProject/CN=TheXPerienceProject"
...
function build_kernel() {
    ...
    local xperience_version="xperience-$(_get_build_var_cached PRODUCT_VERSION_MAJOR).$(_get_build_var_cached PRODUCT_VERSION_MINOR)"
    ...
    if [[ "${SKIP_KERNEL_SYNC}" != "true" && "${SKIP_KERNEL_SYNC}" != "1" ]]; then
        ...
        local repo_init_args=("-b" "${xperience_version}")
