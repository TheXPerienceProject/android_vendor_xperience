...
function cafmerge()
{
    ...
    if (echo $PROJECT | grep -qv "^vendor")
	then 
    ...
    if (echo $PROJECT | grep -q "device/qcom/common")
	then
	    local BRANCH=${VENDORBRANCH}
		echo "Setting VENDOR branch"
	fi
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.xperience.device | grep -q "$XPERIENCE_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.xperience.device | grep -q "$XPERIENCE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
