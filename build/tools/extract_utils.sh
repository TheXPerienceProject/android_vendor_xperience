...
function setup_vendor() {
    ...
    export ANDROIDBP="$XPERIENCE_ROOT"/"$OUTDIR"/Android.bp
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        ...
        printf " * Copyright (C) 2019-$YEAR The XPerience Project\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR The XPerience Project\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The XPerience Project\n" >> $1
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$XPERIENCE_ROOT"/prebuilts/tools-lineage/common/smali/baksmali.jar
        export SMALIJAR="$XPERIENCE_ROOT"/prebuilts/tools-lineage/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$XPERIENCE_ROOT"/prebuilts/tools-lineage/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$XPERIENCE_ROOT"/prebuilts/tools-lineage/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
