...
func init() {
	android.RegisterModuleType("xperience_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = xperienceExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = xperienceExpandVariables(ctx, dep_file)
	...
	exCmd := xperienceExpandVariables(ctx, String(g.properties.Cmd))
	...
	manifestName := "xpe.sbox.textproto"
	desc := "generate"
        name := "generator"
